...
def validate_bed_content(bed):
    with open(bed) as file:
        ...
        if not re.search(r'^(?:[Cc][Hh][Rr])?([XxYy]|[1-9]|1\d|2[012])$', line[0]):
            raise ValueError(f'Chromosome format incorrect: {line[0]}')
        ...
        if int(line[2]) < int(line[1]):
            raise ValueError(f'Difference between start coordinate and stop coordinate '
                             f'must be less than 10000. Difference: {int(line[2]) - int(line[1])}')
